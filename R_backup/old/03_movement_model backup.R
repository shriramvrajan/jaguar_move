...
loglike_fun <- function(par) {
  ...
  # Probability here is basically the utility function for each cell of nbhd
  ...
  for (j in 1:(steps - 1)) {
    ...
    step_prob <- as.vector(current[, , j]) * probability[]
...
for (i in i_initial:n_iter) {
  ...
  # extended neighborhood, as represented by a cell number of brdf
  ...
  # Reindexing allows linkage of row numbers from nbhd to brdf cells
