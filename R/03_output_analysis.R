...
llk <- -runk[[1]]; llrw <- -runrw[[1]]
park <- runk[[2]]; parrw <- runrw[[2]]

null_files <- paste0("null_", 1:njag, ".RDS")
ll0 <- -unlist(load_if_exists(null_files, "data/output/"))

dfk <- par_to_df(park)
dfrw <- par_to_df(parrw)

nmove <- sapply(1:njag, function(x) length(which(jag_move$ID == jag_id[x])))
ndays <- sapply(1:njag, function(x) {
    moves <- jag_move[which(jag_move$ID == jag_id[x]), ]
