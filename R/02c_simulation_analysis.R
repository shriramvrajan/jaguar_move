if (!exists("loaded")) source("R/00_functions.R")
loaded <- TRUE
...
nbhd_i <- make_nbhd(i = path_i$cell, sz = buffersize, r = env01[[1]], rdf = env01[[2]])
...
nbhd <- nbhd_i
att_true <- 1 / (1 + exp(par_true[1] + par_true[2] * env_i + par_true[3] * env_i^2))
att_true <- att_true / rowSums(att_true)
att_fit <- 1 / (1 + exp(par_fit[1] + par_fit[2] * env_i + par_fit[3] * env_i^2))
...
par(mfrow = c(2, 1))
plot_the_curve(par_true)
plot_the_curve(par_fit)

par(mfrow = c(1, 1))
plot(env_i, att_true, pch = 19, col = rgb(0, 0, 0, 0.2), cex = 0.5, ylim = c(0, 0.2))
...
log_likelihood(par_true, objects1)
log_likelihood(par_fit, objects1)
